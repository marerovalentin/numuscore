			...
			const string strMessageMagic = "Numus Signed Message:\n";
			...
			int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
			{
				...
				else if (nHeight > 100 * 1000 && nHeight <= 400 * 1000) {
					nSubsidy = 0 * COIN;
				}
				else if (nHeight > 400 * 1000 && nHeight <= 500 * 1000) {
					nSubsidy = 1.5 * COIN;
				}
				else if (nHeight > 500 * 1000 && nHeight <= 600 * 1000) {
					nSubsidy = 1 * COIN;
				}
				else if (nHeight > 600 * 1000 && nHeight <= 700 * 1000) {
					nSubsidy = 0.75 * COIN;
				}
			...
			int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
			{
				...
				else if (pindexBest->nHeight+1 > 200000 && pindexBest->nHeight+1 <= 210000) {
					nSubsidy = 6 * COIN;
				}
				else if (pindexBest->nHeight+1 > 210000 && pindexBest->nHeight+1 <= 250000) {
					nSubsidy = 5 * COIN;
				}
				else if (pindexBest->nHeight+1 > 250000 && pindexBest->nHeight+1 <= 300000) {
					nSubsidy = 4 * COIN;
				}
				else if (pindexBest->nHeight+1 > 300000 && pindexBest->nHeight+1 <= 350000) {
					nSubsidy = 3 * COIN;
				}
				else if (pindexBest->nHeight+1 > 350000 && pindexBest->nHeight+1 <= 400000) {
					nSubsidy = 2 * COIN;
				}
			...
			bool IsInitialBlockDownload()
			{
				...
				return (GetTime() - nLastUpdate < 15 &&
						pindexBest->GetBlockTime() < GetTime() - 5400);
			}
			...
			// ----------- instantX transaction scanning -----------
				...
				if(hasPayment && payeerewardpercent == 0){
					...
					CNumuscoinAddress address2(address1);
				...
				if(hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100){
					...
					CNumuscoinAddress address4(address3);
					targetNode = address2.ToString().c_str();
			...
			bool CBlock::AcceptBlock()
			{
				...
				uint256 hashProof;
				if (IsProofOfWork() && nHeight > Params().LastPOWBlock() && nHeight < Params().POWRestartBlock()) {
